...
@testset "Result containers" begin
    ...
    𝒯ᴿᴴ = collect(T)[indices_optimization(first(model.horizons))]
    case_RH, model_RH = get_RH_case_model(case, model, 𝒯ᴿᴴ)
    ...
    EMRH.update_results!(results_EMRH, m_RH1, case_RH, case, 𝒯ᴿᴴ)
